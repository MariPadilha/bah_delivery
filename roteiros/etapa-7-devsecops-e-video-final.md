...
| 4 | Testes automatizados |  Execução dos testes de segurança definidos antes da implementação na Etapa 4: **TS01** e **TS02** para a busca de restaurantes, e **TS03** a **TS06** para autorização, que cobrem os três critérios de RS03 — negação por padrão, descarte do perfil enviado pelo cliente e recusa de token inválido  | Resultado da suíte automatizada, identificando os testes executados e seus resultados, incluindo evidência de pesquisa válida, tentativa de injeção tratada como dado, acesso legítimo permitido, operação não autorizada recusada, perfil descartado no cadastro e token inválido rejeitado | TS01 a TS06 devem ser aprovados, e cada critério de RS03 precisa ter teste automatizado próprio antes da promoção |
...
TS03 e TS04 verificam a autorização e a negação por padrão, mas não alcançam sozinhos os outros dois critérios de **RS03**. Por isso a Etapa 4 define **TS05** e **TS06**: o primeiro envia um campo `perfil` com o valor `administrador` no cadastro e verifica que a conta nasce como `cliente` e não alcança operação administrativa; o segundo recusa token forjado, expirado e com algoritmo alterado, e confirma que o perfil usado na decisão vem da conta armazenada, ainda que o token declare outro. Os dois existem porque a leitura do código não substitui a evidência produzida pela execução desses casos — é a mesma razão pela qual a condição 6 da seção 3 exige teste próprio para cada critério, e não apenas para o conjunto.
...
A evidência deste momento é o resultado registrado da suíte automatizada, indicando os casos executados e seus resultados. A continuidade exige a aprovação de **TS01 a TS06**. Uma falha devolve o código ao momento de implementação, em vez de permitir que uma propriedade de segurança não atendida avance para as etapas seguintes do pipeline.
...
O momento também fecha um ciclo com o momento 4. Os testes TS01 a TS06 exercitam as práticas da Etapa 4 em nível de unidade, sobre a função; aqui os mesmos comportamentos são exercitados sobre o sistema em execução, por requisição HTTP. A diferença é o que está sendo verificado: lá, que a função recusa a entrada; aqui, que a rota que a chama efetivamente a chama.
...
5. Reprovação de qualquer teste de TS01 a TS06 (momento 4). A falha devolve o código à implementação, em vez de permitir que uma propriedade de segurança não atendida avance.
6. Critério de RS03 sem teste automatizado próprio (momento 4). Os três critérios estão cobertos hoje — negação por padrão por TS03 e TS04, descarte do campo `perfil` (**C67**) por TS05 e recusa de token inválido (**C68**) por TS06 —, e a condição permanece porque vale para todo critério acrescentado depois: a leitura do código não substitui a evidência produzida pela execução do caso.
